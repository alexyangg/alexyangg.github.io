/**
 * Ensures deterministic scroll and hash restoration across reloads,
 * back/forward navigation, and bfcache restores in a Next.js app.
 *
 * - Manually saves scroll position and URL hash to sessionStorage
 * - Disables browser scroll restoration for consistency
 * - Waits for hydration/layout to settle before restoring scroll
 * - Normalizes the hash to #home when near the top
  ...
  useEffect(() => {
    ...
    const save = () => {
      try {
        ...
        sessionStorage.setItem(KEY_HASH, window.location.hash || "#home");
      } catch {}
    };
    ...
    const restore = async () => {
      ...
      try {
        ...
        hash =
          sessionStorage.getItem(KEY_HASH) || window.location.hash || "#home";
      } catch {}
      ...
      // If we're basically at top, force hash to #home (prevents "home but hash shows projects")
